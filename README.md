...
web/
  ...
  js/v20260910k/data.js            常量与中文映射（TILE/LAYERS/BLOCK_CN/CONTENT_CN/…）
  js/v20260910k/cn_data.js         全量官方中文名（由 bundle_zh_CN.properties 生成）
  js/v20260910k/inflate.js         zlib 解压封装（DecompressionStream）
  js/v20260910k/parser.js          容器解析 + TypeIO + contentMap + 处理器逻辑提取
  js/v20260910k/render.js          渲染器（与 msch.py 像素级一致）
  js/v20260910k/icons.js           PUA 内容图标解析 + richText 富文本
  js/v20260910k/icons_data.js      PUA 码点表（由 icons.properties 生成）
  js/v20260910k/prefetch.js        输入哈希 + 预加载管理器（可单测）
  js/v20260910k/cache.js           Cache Storage 持久化缓存（v2 + 规范化键 + SWR/TTL）
  js/v20260910k/sources.js         Mindustry 镜像源（手动选择 + 超时 + 自动切换）
  js/v20260910k/zip.js             纯 JS zip 读取器（STORED/DEFLATE，可单测）
  js/v20260910k/mod.js             模组 zip 解析（方块/贴图/bundle，可单测）
  js/v20260910k/names.js           方块显示名（bundle > JSON name > BLOCK_CN，可单测）
  js/v20260910k/history.js         本地历史记录（容量策略纯函数，可单测）
  js/v20260910k/requirements.js    蓝图总耗材计算（可单测）
  js/v20260910k/requirements_data.js 方块耗材表 BLOCK_REQUIREMENTS + 物品中文名 ITEM_CN
  js/v20260910k/main.js            入口 UI 逻辑（js 目录随发布整体版本化，路径级缓存穿透）
...
2. **上传 `web/` 里的内容**：把 `web/` 目录**里面的全部文件**上传到仓库根目录，
   使仓库根目录下直接有 `index.html`、`css/`、`js/v20260910k/`、`sprite_index.json` 等。
...
> 且 `?v=` 对部分按路径缓存的代理无效。为彻底穿透缓存，**整个 js 目录随发布版本化**。每次发布请：
> 1. 在 `index.html` 顶部注释中**递增版本号 VER**；
> 2. 把整个 js 目录移动到新版本路径：`git mv js js/v<VER>`（入口为 `js/v<VER>/main.js`）。
>    旧路径（如 `js/data.js`）随即 404，任何浏览器/代理缓存都无法再命中旧模块；
...
- **指向别的 CDN / 镜像**：编辑 `js/v20260910k/sources.js` 的 `SOURCES`（按优先级排列），
  或删除不需要的源；运行时会自动切换并记住最近可用源。
- **改单个贴图路径**：编辑 `sprite_index.json`（或 `js/v20260910k/data.js` 的 `AUX_PATHS`）。
  索引结构为：
  ```json
  {
...
1. **内容图标**：物品 / 液体 / 方块 / 单位等，由 `icons/icons.properties`
   ...
   - 数据来源：`icons.properties` → `js/v20260910k/icons_data.js`（`ICON_BY_CODE`，626 条）。
   - 解析：`js/v20260910k/icons.js` 的 `resolveIcon(code)`；区域名去掉结尾 `-ui` 得 base，
...
- **全量官方中文名 `js/v20260910k/cn_data.js`**：由官方 `bundle_zh_CN.properties` 生成
  （`CN_BLOCKS` 412 条 / `CN_ITEMS` 22 条 / `CN_LIQUIDS` 11 条）。`js/v20260910k/data.js` 的
...
- **原版图标 `assets/icons/<码点>.png`**：从官方 `assets.jar` 导出 530 个 PUA 内容图标，
  ...
  `js/v20260910k/icons_data.js` 的 `ICON_LOCAL_CODES` 记录这些码点；`richText` 对命中码点优先使用
...
- **耗材 `js/v20260910k/requirements_data.js`**：`BLOCK_REQUIREMENTS`（231 个方块）来自 Mindustry
  `Blocks.java` 的 `requirements(...)`，与游戏 `Schematic.requirements()` 一致
  （把每个方块的 requirements 直接累加，无倍率）；`ITEM_CN` 为官方中文名。
  渲染完成后，`js/v20260910k/requirements.js` 的 `computeRequirements()` / `requirementsList()`
  累加并在页面显示「耗材」面板（物品图标 + 中文名 + ×数量，按数量降序）。
- **配置影响贴图（v159.7 源码行为）**：`js/v20260910k/data.js` 用 `CONFIG_UNDERLAY` / `CONFIG_OVERLAY`
...
- **解析**（`js/v20260910k/mod.js` + `js/v20260910k/zip.js`，纯 JS、无第三方库）：
...
- **方块显示名优先级**：模组语言文件 `block.<内部名>.name`（带 `<mod>-` 前缀时也尝试
  ...
  图例、热区提示、处理器按钮等均使用该显示名（`js/v20260910k/names.js` 的 `blockDisplayName`）。
...
- **渲染集成**：贴图查找顺序为 内存 → 模组 `sprites-override` → 本地 `assets/sprites` →
  ...
  时按该尺寸占位。
- **模组多层 = 按 JSON `drawer` 绘制栈解析**（`js/v20260910k/mod.js` 的 `drawerStaticLayers`）：
...
- 预加载由 `js/v20260910k/prefetch.js` 的 `createPrefetchManager` 管理：按输入哈希复用已解析结果，
...
- 容量策略（`js/v20260910k/history.js` 纯函数）：单条 `input > 1MB` 不记录；总条数 ≤ **16**；
...
- `js/v20260910k/cache.js` 用 `caches.open("msch-cache-v2")` 缓存贴图与 `sprite_index.json`：
...
```bash
cd web
node --check js/v20260910k/data.js js/v20260910k/cn_data.js js/v20260910k/inflate.js js/v20260910k/parser.js js/v20260910k/render.js js/v20260910k/icons.js js/v20260910k/icons_data.js js/v20260910k/prefetch.js js/v20260910k/cache.js js/v20260910k/sources.js js/v20260910k/zip.js js/v20260910k/mod.js js/v20260910k/requirements.js js/v20260910k/requirements_data.js js/v20260910k/names.js js/v20260910k/history.js js/v20260910k/main.20260910i.js
