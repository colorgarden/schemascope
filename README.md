...
> 每次发布请把版本号 VER 在**这些地方**同步递增：
> 1. `index.html` 顶部注释（`VER: <VER>`）；
> 2. 入口 `js/main.js?v=<VER>` 与 `css/style.css?v=<VER>`；
> 3. `js/main.js` 内的 `APP_VERSION`（页脚版本由 JS 注入，能显示即证明新脚本已加载）；
> 4. **`js/*.js` 内部所有相对 import 的 `?v=<VER>`**，一条命令：
>    `sed -i 's|\.js?v=[0-9a-z]*"|.js?v=<VER>"|g' js/*.js`
>    （保证入口与各模块同版本，避免「新入口 + 旧缓存模块」混用导致整个应用报错）；
> 5. 若用户仍看到旧版本或异常，请其清理浏览器缓存（或换浏览器/无痕模式）后重试。
